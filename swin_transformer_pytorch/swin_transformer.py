...
# 输出的shape 是 q: batch_size,
        ...
        if self.shifted: # 加上偏移的mask
            ...
            # 然后里面每一个东西.都是一个7*7的.因为是左边roll过来的.所以 这个7*7 最后3列是要mask的部分. # 要自己算一下2维矩阵7*7拍平成49之后的样子. 因为roll,所以还是先4, 后3.
        ...
        if self.shifted: # 往右滚就不用管mask了. 为了之后的保持回去. 保证每一轮迭代的最开始都是初始图像.体现了论文中 奇数偶数的layer设置.
...
class StageModule(nn.Module):
    ...
    def forward(self, x):
        x = self.patch_partition(x)  # 变到 56 * 56 了. 把图片4个像素变成一个patch    224=56*4
